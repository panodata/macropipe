...
class Functions:
    ...
    def concat(
        ...
        lf = self._lf.with_columns(
            pl.concat_str(column_names, separator=separator).alias(target_column)
        )
    ...
    def filter(self, clause: str) -> pl.LazyFrame:
        """
        ...
        """
        return self._lf.filter(pl.sql_expr(clause))
    ...
    def json_array_to_wkt_point(self, col_name: str) -> pl.LazyFrame:
        ...
        """
        import polars_st as st  # ty: ignore[unresolved-import, unused-ignore-comment, unused-ignore-comment]
    ...
    def columns_to_json_array(
        self,
        source_columns: t.Union[str, t.List[str]],
        target_column: str,
        options: t.Optional[str] = None,
    ) -> pl.LazyFrame:
        """
    ...
    def json_fields_to_columns(
        ...
        lf = self._lf.with_columns(
            *(decoded.struct.field(col).alias(col) for col in extract_columns)
        )
    ...
    @staticmethod
    def _drop_if_requested(
        lf: pl.LazyFrame, options: t.Optional[str], columns: t.List[str]
    ) -> pl.LazyFrame:
