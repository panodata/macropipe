...
from macropipe import MacroPipe, recipe
from macropipe.lib import Functions
...
def test_lib_drop_if_requested():
    ...
    converted_frame = Functions._drop_if_requested(input_frame, "", ["b", "c"])
    ...
    converted_frame = Functions._drop_if_requested(input_frame, "drop=true", ["b", "c"])
...
def test_user_registered_recipe():
    ...
    def hello(lf: pl.LazyFrame, column_name: str) -> pl.LazyFrame:
        return lf.with_columns(
            pl.concat_str(pl.lit("Hello"), pl.col(column_name), separator=" ").alias(column_name)
        )
    ...
    output_frame = pl.LazyFrame(
        {
            ...
            "coords": ["POINT (9.757 47.389)"],
            "data": ['{"temperature":42.42}'],
        }
    )
    pipe = MacroPipe.from_recipes(
...
1754784000000,'{"longitude": 9.757, "latitude": 47.389}'
    ...
    output_frame = pl.LazyFrame(
        {"timestamp": [1754784000000], "longitude": [9.757], "latitude": [47.389]}
    )
    converted_frame = pipe.apply(input_frame)
    assert_frame_equal(converted_frame, output_frame)
